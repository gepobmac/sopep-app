...
type Role = 'desenvolvedor'|'fiscalizacao'|'operacao'|'administracao';
...
async function init() {
  ...
  try {
    ...
    CURRENT_ROLE = me.role as Role;
    ($('#userBadge') as HTMLElement).textContent = `${me.name} — ${me.role}`;
  } catch {
    location.href = '/';
    return;
  }
  ...
  $$('.nav [data-view]').forEach(btn=>{
    const roles = (btn.getAttribute('data-roles') || 'desenvolvedor,fiscalizacao,operacao,administracao')
      .split(',').map(s=>s.trim());
    if(!roles.includes(CURRENT_ROLE)) (btn as HTMLButtonElement).style.display = 'none';
  });
...
async function routeTo(view: string) {
  hideAll();
  if(view === 'dashboard') {
    ($('#view-dashboard') as HTMLElement).removeAttribute('hidden');
  ...
  if(view === 'aprovacoes') {
    ...
    $$('#view-aprovacoes [data-act]').forEach(b=>{
      b.addEventListener('click', async ()=>{
        ...
        await Data.decidir(id, decisao as any);
  ...
  if(view === 'estoque') {
    ...
    $$('#view-estoque [data-mov]').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        ...
        const tipo = btn.getAttribute('data-mov') as 'entrada'|'saida';
        const qtd = Number((document.getElementById(`q-${id}`) as HTMLInputElement)?.value || '0');
  ...
  if(view === 'kits') {
    ...
    renderViewContainer('kits', `
      ...
      <table>
        ...
        <tbody>${kits.map((k:any)=>`<tr><td>${k.nome}</td><td>${k.local}</td><td>${k.ativo?'Ativo':'Inativo'}</td></tr>`).join('')}</tbody>
      </table>
    `);
    return;
  }
  ...
  if(view === 'relatorios') {
    ...
    $$('#view-relatorios [data-act]').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        const id = btn.getAttribute('data-id')!;
        const act = btn.getAttribute('data-act') as 'comprado'|'fechar';
